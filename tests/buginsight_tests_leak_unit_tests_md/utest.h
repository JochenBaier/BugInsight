...
extern void* allocator_malloc(size_t _Size);
extern void allocator_free(void* _Block);
extern void* allocator_realloc(void* _Block, size_t _Size);
...
{
  void* const new_pointer=allocator_realloc(pointer, new_size);

  if(UTEST_NULL == new_pointer) {
    allocator_free(new_pointer);
  }
  ...
  UTEST_SURPRESS_WARNING_BEGIN do {                                            \
    ...
    try {                                                                      \
      ...
      if (0 !=                                                                 \
        ...
        message_caught = UTEST_PTR_CAST(char *, allocator_malloc(message_size));         \
...
#define UTEST(SET, NAME)                                                       \
  ...
  UTEST_INITIALIZER(utest_register_##SET##_##NAME) {                           \
    ...
    char *name = UTEST_PTR_CAST(char *, allocator_malloc(name_size));                    \
...
cleanup:
  for(index=0; index < utest_state.tests_length; index++) {
    allocator_free(UTEST_PTR_CAST(void*, utest_state.tests[index].name));
  }

  allocator_free(UTEST_PTR_CAST(void*, skipped_testcases));
  allocator_free(UTEST_PTR_CAST(void*, failed_testcases));
  allocator_free(UTEST_PTR_CAST(void*, utest_state.tests));
